defmodule AdventOfCode2021.Puzzles.Day7.Part2 do
  ...
  def main() do
    load(:real)
    # |> Debug.tap(fn numbers ->
    #   0..16
    #   |> Enum.each(&("#{&1} ==> #{numbers |> get_score(&1)}" |> IO.puts()))
    # end)
    |> find_max_and_min_with_scores()
    |> find_preferred_position()
    |> Debug.tee()
    |> (fn {_at, score } -> score end).()
  end
  ...
  defp find_preferred_position({ numbers, min_pt = { min_v, _score_at_min }, max_pt = { max_v, _score_at_max }}) do
    { low_mid, high_mid } = cond do
      max_v - min_v == 2 -> { min_v + 1, min_v + 1 }
      true -> mid_points(min_v, max_v)
    end
  ...
  defp get_score(numbers, at_point) do
    ...
    |> Enum.map(&calc_score_for_distance/1)
    |> Enum.sum()
  end

  defp calc_score_for_distance(distance) when distance == 0, do: 0
  defp calc_score_for_distance(distance), do: distance + calc_score_for_distance(distance - 1)
